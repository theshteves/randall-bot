...
slack.on('message', function(message) {
	...
	var inviteUser = function(person_id) {
		...
		if (!error && response.statusCode == 200) {
		    console.log(body);
		    channel.send(":yoshiegg: " + slack.getUserByID(person_id) + " has been summoned :yoshiegg:");
		}
	    })
	    return true;
	};
	...
	if (message.text.substring(0,7) == ":yoshi:" && isUser(message.text.substring(8))) {
		...
		if (message.text.substring(8) == "durr") {
		    ...
		    mainland[message.text.substring(8)] = [[], island[message.text.substring(8)][2]];
		    kickUser(island[message.text.substring(8)][2]);
		    delete island[message.text.substring(8)];
		}
	    }
	}
